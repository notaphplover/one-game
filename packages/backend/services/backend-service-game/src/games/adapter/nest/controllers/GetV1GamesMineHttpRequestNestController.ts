...
import { GetV1GamesMineHttpRequestController } from '@cornie-js/backend-game-application/games';
import {
...
@Controller('v1/games')
export class GetV1GamesMineHttpRequestNestController extends HttpNestFastifyController<Request> {
  constructor(
    @Inject(RequestFromFastifyRequestBuilder)
    requestBuilder: Builder<Request, [FastifyRequest]>,
    @Inject(GetV1GamesMineHttpRequestController)
