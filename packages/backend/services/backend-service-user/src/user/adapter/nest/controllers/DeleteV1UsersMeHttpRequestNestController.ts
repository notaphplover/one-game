...
import { DeleteV1UsersMeHttpRequestController } from '@cornie-js/backend-user-application/users';
import { Controller, Delete, Inject, Req, Res } from '@nestjs/common';
import { FastifyReply, FastifyRequest } from 'fastify';

@Controller('v1/users/me')
export class DeleteV1UsersMeHttpRequestNestController extends HttpNestFastifyController<Request> {
  constructor(
    @Inject(RequestFromFastifyRequestBuilder)
    requestBuilder: Builder<Request, [FastifyRequest]>,
    @Inject(DeleteV1UsersMeHttpRequestController)
