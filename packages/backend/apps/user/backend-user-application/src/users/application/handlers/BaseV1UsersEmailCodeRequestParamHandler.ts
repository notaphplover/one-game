...
export abstract class BaseV1UsersEmailCodeRequestParamHandler {
  ...
  protected async _getUserByRequestOrThrow(request: Request): Promise<User> {
    ...
    if (emailUrlParam === undefined) {
      throw new AppError(
        ...
        `Unexpected missing "${BaseV1UsersEmailCodeRequestParamHandler.emailUrlParameter}" url parameter`,
      );
    }
